...
# Input values are a.disc and ang.disc which discretize the ranges a2,a3 in [0,100] for
# a.disc, and cp in [0,1] and theta in [0,pi/2] for ang.disc. The entire muscape will
# be generated in pieces, so we allow a2.inds and a3.inds to specify which values are
# done. Note that a2.inds must be in (0,100]/a.disc and a3.inds in [0,a2.inds].

# The resulting data frame contains the parameters, the mu, the corresponding 
# horizon co-ordinates, and the horizon magnitude

muscape.generate <- function(a.disc,ang.disc,a2.inds,a3.inds){
  ...
  no.pts = ang.disc^2*ac
  
  # Initialize data frame
  muscape = data.frame(a2=double(no.pts),a3=double(no.pts),cp=double(no.pts),
         theta=double(no.pts),mu=double(no.pts),nM1=double(no.pts),nM2=double(no.pts),
         nM3=double(no.pts),nMmag=double(no.pts))
  count = 1

  # We need the functions horizon.find() and b.from.angles()
  source("horizon.find.R")
  
  # Sweep over a2, a3 indices
  for (j1 in a2.inds){
    for (j2 in a3.inds){
      a=c(100,100/a.disc*j1,100/a.disc*j2)
      
      # Sweep over angles
      for (j3 in 0:ang.disc){
        for (j4 in 0:ang.disc){

          # Calculate data
          cp = j3/ang.disc
          th = j4/ang.disc/2*pi
          b= b.from.angles(a,cp,th,1)
          mus = horizon.find(a,b)
...
muscape.add.hams <- function(msc){
  ...
  for (j in 1:nrows){
    ...
    b = b.from.angles(a, msrow[3],msrow[4],1)
    msc[j, 10:12] = h.from.n(msrow[6:8], a, b)
  }
  return(msc)
}