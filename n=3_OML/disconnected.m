...
if (w12==w21)
	...
	% Plot the GC set
	fill(4/sqrt(2)*[(R+1)/2,R,(R-1)/2,-(R-1)/2,-R,-(R+1)/2,(R+1)/2]/(1+3*R^2), ...
		4/sqrt(2)*[(3*R-1)/2,1,(-3*R-1)/2,(-3*R-1)/2,1,(3*R-1)/2,(3*R-1)/2]...
		/(1+3*R^2)/sqrt(3),'g');

	% Plot the STLC set
	fill([2*(R+1)/(1+3*R^2),-2/(1-3*R),4*R/(1+3*R^2),2/(1+3*R),2*(R-1)/...
		(1+3*R^2),0,-2*(R-1)/(1+3*R^2),-2/(1+3*R),-4*R/(1+3*R^2),2/...
		(1-3*R),-2*(R+1)/(1+3*R^2),0,2*(R+1)/(1+3*R^2)]/sqrt(2),[2*(3*R-1)/...
		...
		/sqrt(3),2*(3*R-1)/(1+3*R^2)/sqrt(3)]/sqrt(2),'b');


	% Plot the six lines that make up the set boundaries
	plot([1,0,-1]/sqrt(2)/R,[1,-2,1]/sqrt(2)/sqrt(3),'b');
	plot([1/2-1/2/R,-1,1/2+1/2/R]/sqrt(2),[-1/2-3/2/R,1,-1/2+3/2/R]/sqrt(2)/sqrt(3),'b');
	plot(-[1/2-1/2/R,-1,1/2+1/2/R]/sqrt(2),[-1/2-3/2/R,1,-1/2+3/2/R]/sqrt(2)/sqrt(3),'b');
